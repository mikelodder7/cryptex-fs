...
impl KeyRing for KeyRingFile {
    fn get_secret<S: AsRef<str>>(&mut self, id: S) -> cryptex::Result<KeyRingSecret> {
        ...
        let opt_ciphertext_and_enc_key =
            self.db
                .get(&rtxn, id)
                .map_err(|e| KeyRingError::GeneralError {
                    msg: format!("{:?}", e),
        ...
        match self.decryption_key.unprotect() {
            None => Err(KeyRingError::AccessDenied {
                msg: String::from("decryption key has been tampered"),
            }),
            Some(secret_bytes) => {
                ...
                let mut output = derive_symm_key(
                    ...
                    &eph_pub_key,
                    &self.encryption_key,
                    secret_key.diffie_hellman(&eph_pub_key),
                );
                ...
                let plaintext = cipher
                    .decrypt(&nonce, &ciphertext_and_enc_key[32..])
                    .map_err(|e| KeyRingError::GeneralError {
                        msg: format!("{:?}", e),
                    })?;
                output.zeroize();
        ...
        if let Some(e) = self.entropy {
            ...
            match (self.password, self.password_salt, self.password_hash) {
                (Some(ref mut pass), Some(salt), Some(hasher)) => {
                    ...
                    let mut hash_bytes =
                        <[u8; KeyRingFileOpenOptions::DEFAULT_OUT_LEN]>::try_from(hash.as_bytes())
                            .unwrap();
                    let mut scalar =
